...
export function About() {
  return (
    <section id="sobre" className="mx-auto max-w-[1180px] px-5 py-[clamp(52px,7vw,88px)]">
      <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-[60px]">
        <div>
          <Reveal>
            ...
            <p className="mt-[18px] text-[16.5px] leading-[1.7] text-ink-700">
              São mais de {siteConfig.yearsInMarket} anos no mercado de seguros construindo
              relações de confiança com famílias e empresas. Cuidamos de todo o processo:
              entendemos sua necessidade, comparamos as seguradoras, explicamos cada cobertura em
              linguagem simples e acompanhamos você no momento em que mais importa — o do
              sinistro.
            </p>
          </Reveal>
          <div className="mt-[26px] flex flex-col gap-3.5">
            {VALUES.map((v, i) => (
              <Reveal key={v.title} as="span" delay={120 + i * 90} className="flex items-start gap-3">
                <Icon
                  name="checkmark"
                  size={20}
                  strokeWidth={2}
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(140px,1fr))] gap-3.5">
          {siteConfig.stats.about.map((stat, i) => (
            <Reveal
              key={stat.label}
              delay={i * 90}
              className="rounded-2xl border border-ink-200 bg-ink-50 p-[26px]"
            >
